...
class MySolution(BlockSelection, RL):
    ...
    def select_block(self, cur_time, block_queue):
        ...
        def is_better(block):
            ...
            asume_time = self.average_RTT / 2
            asume_rate = self.send_rate

            best_block_priority = best_block.block_info["Priority"] + 1
            block_priority = block.block_info["Priority"] + 1
            ...
            packet_ability = block_priority * abs(block.block_info["Deadline"] - packet_ability)

            best_packet_ability = cur_time - best_block_create_time + asume_time + best_block_remain_packets / asume_rate
            best_packet_ability = best_block_priority * abs(best_block.block_info["Deadline"] - best_packet_ability)
