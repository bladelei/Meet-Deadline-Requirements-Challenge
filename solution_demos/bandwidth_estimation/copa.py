...
class MySolution(Packet_selection, Reno):
    ...
    def select_packet(self, cur_time, packet_queue):
        ...
        :return: int
        """
        def is_better(packet, best_packet):
            best_block_create_time = best_packet.block_info["Create_time"]
            packet_block_create_time = packet.block_info["Create_time"]

            best_block_size = int(best_packet.block_info["Size"]) // 1480
            packet_block_size = int(packet.block_info["Size"]) // 1480
            ...
            if packet_time_left < 0:
                return False
            else:
                if self.rtt_window.get_unjitter_rtt() > 0:
                    if best_time_left < self.rtt_window.get_unjitter_rtt():
                        return True
                    if (packet_time_left > self.rtt_window.get_unjitter_rtt()) and (best_time_left > packet_time_left):
                        return True
                    else:
                        return False
                else:
                    if packet_time_left < best_time_left:
                        return True
                    else:
                        return False
        ...
        for idx, item in enumerate(packet_queue):
            if best_packet is None:
                if item.block_info["Deadline"] - (cur_time - item.block_info["Create_time"]) > 0:
                    best_packet_idx = idx
                    best_packet = item
                else:
                    continue

            if is_better(item, best_packet):
                best_packet_idx = idx
                best_packet = item
            block_list.add(item.block_info["Block_id"])

       # print("block_count:",len(block_list))
    ...
    def cc_trigger(self,data):
        ...
        rtt = data["packet_information_dict"]["Pacing_delay"] + data["packet_information_dict"]["Latency"]+data["packet_information_dict"]["Send_delay"]
